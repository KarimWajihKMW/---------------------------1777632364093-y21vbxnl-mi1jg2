...
function adminSupportPage() {
  const publicRequests = window.getPublicSupportRequests?.() || [];
  ...
  const { pages, visible } = getPagedItems(list);
  return `${listToolbar({ placeholder: 'ابحث في مشاكل الدعم أو العملاء', filters: statuses, activeFilter, activeSort, sorts: [{ value: 'status', label: 'الحالة' }, { value: 'priority', label: 'الأولوية' }, { value: 'client', label: 'العميل' }] })}<button onclick="createSupportTicket()" class="mt-4 rounded-2xl bg-moss px-5 py-3 font-extrabold text-white">إضافة مشكلة دعم</button><div class="mt-4 rounded-[2rem] border border-white/70 bg-white/65 p-4 shadow-sm"><h3 class="font-display text-xl font-extrabold text-moss">طلبات التواصل العامة</h3><div class="mt-3 grid gap-2">${publicRequests.slice(0, 4).map((request) => `<div class="rounded-2xl bg-mint/55 p-3 text-sm leading-7"><b>${escapeHTML(request.id)} · ${escapeHTML(request.name)} · ${escapeHTML(request.topic)}</b><p>${escapeHTML(request.phone)} · ${escapeHTML(request.message)}</p><a class="whatsapp-action mt-2 inline-flex rounded-xl px-3 py-2 text-xs font-extrabold" href="${window.whatsappUrl(request.phone, `مرحباً ${request.name}، بخصوص طلبك ${request.id}: `)}" target="_blank" rel="noopener">رد واتساب</a></div>`).join('') || '<p class="text-sm text-ink/60">لا توجد طلبات عامة بعد.</p>'}</div></div><div class="mt-6 grid gap-4">${visible.map(({ ticket, index }) => `<article class="rounded-[2rem] border border-white/70 bg-white/75 p-5 shadow-sm"><div class="flex flex-wrap items-start justify-between gap-3"><div><b class="text-moss">${escapeHTML(ticket.id)} · ${escapeHTML(ticket.client)}</b><p class="mt-2 leading-7 text-ink/65">${escapeHTML(ticket.issue)}</p></div><span class="rounded-full bg-sand px-3 py-1 text-xs font-extrabold text-moss">${escapeHTML(ticket.priority)} · ${escapeHTML(ticket.status)}</span></div><p class="mt-3 rounded-2xl bg-mint/55 p-3 text-sm text-ink/65">الحل: ${escapeHTML(ticket.resolution)}</p><div class="mt-4 flex flex-wrap gap-2"><button onclick="editSupportTicket(${index})" class="table-action rounded-xl bg-mint px-3 py-2 text-xs font-bold text-moss">تعديل الحل</button><button onclick="closeSupportTicket(${index})" class="table-action rounded-xl bg-moss px-3 py-2 text-xs font-bold text-white">إغلاق المشكلة</button><button onclick="deleteSupportTicket(${index})" class="table-action rounded-xl bg-red-50 px-3 py-2 text-xs font-bold text-red-700">حذف</button></div></article>`).join('') || emptyState()}</div>${pagination(pages)}`;
}
